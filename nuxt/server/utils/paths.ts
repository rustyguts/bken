...
// `DATA_DIR`. We resolve them here the same way `vidpipe.config.resolve_data`
...
// `VIDPIPE_DATA` is set by docker-compose; fall back to `../data` so `bun
// run dev` on the host finds the same SQLite + mp4s without configuration.
export const DATA_DIR = resolve(process.env.VIDPIPE_DATA || resolve(process.cwd(), '../data'))
