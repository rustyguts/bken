...
- Drop-aware adaptation loop — capture/playback frame drops feed into bitrate adaptation and quality indicator (adaptBitrateLoop calls DroppedFrames() every tick; capture drops treated as send-side loss equivalent for bitrate step-down; total drops factor into qualityLevel thresholds — >=1/s moderate, >=5/s poor; transport GetMetrics returns playback drops; MetricsBar shows drop count with warning color; 5 new tests for drop-aware quality levels and transport drop counter reset)
- Fast-convergence adaptation — optimistic LAN startup with graduated intervals (default jitter depth lowered from 3 frames/60ms to 1 frame/20ms for minimal LAN latency; fast-start adaptation loop uses graduated intervals 1s→2s→3s→5s steady-state instead of fixed 5s, converging to optimal settings in ~6s instead of ~15s; warmup EWMA alpha 0.5 for first 3 ticks gives faster initial loss convergence before settling to steady-state 0.3; capture loss calculation scaled dynamically by actual interval duration; 7 new tests for LAN-optimistic defaults, warmup convergence speed, fast-start interval ordering, and warmup parameter validation)
