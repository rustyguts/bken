"""vidpipe — gaming video funny-moment extraction pipeline.
...
archives. Designed for archives with thousands of recordings:
    ...
    vidpipe ingest /mnt/shack/media/gaming/archive --workers 8
...
Command groups:
  ...
  vidpipe ingest|pipeline|...     stage commands (flat for muscle memory)
  vidpipe videos ...              browse / inspect / reset indexed videos
  vidpipe clips ...               browse / open candidate clips
  vidpipe db ...                  schema init / vacuum / reset
  vidpipe web                     (moved — see `nuxt/`; start with docker compose up)
...
def cmd_library_add(
    ...
    console.print("  Run [cyan]vidpipe library sync[/] to scan it now.")
...
def cmd_reprocess(
    ...
    Examples:
        vidpipe reprocess score              # re-score + re-clip + re-rank
        vidpipe reprocess vision --skip-rank # re-run vision, score, clip
        vidpipe reprocess audio --yes        # full reprocess from scratch
    """
...
def cmd_db_reset(
    ...
    for ext in ("-wal", "-shm"):
        ...
        if side.exists():
            side.unlink()
    console.print("[green]✓[/] DB reset. Run [cyan]vidpipe db init[/] to recreate.")
...
def cmd_show_clips_alias(
    ...
    """Alias for `vidpipe clips show <video_id>`."""
...
def cmd_stats() -> None:
    ...
    console.print(Panel(overview, title="[bold cyan]vidpipe stats", border_style="cyan"))
...
def cmd_config() -> None:
    ...
    rows = [
        ...
        ("VIDPIPE_CLAUDE_CLI", os.environ.get("VIDPIPE_CLAUDE_CLI", "claude")),
        ("VIDPIPE_CLAUDE_MODEL", os.environ.get("VIDPIPE_CLAUDE_MODEL", "sonnet")),
    ]
    for k, val in rows:
        t.add_row(str(k), str(val))
    console.print(Panel(t, title="[bold cyan]vidpipe config", border_style="cyan"))
