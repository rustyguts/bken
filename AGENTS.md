...
`vidpipe` is a Python CLI pipeline that extracts funny / memorable moments from gameplay recordings. It combines speech-to-text (Whisper), audio event detection (PANNs for laughter/shouting/cheering), and optional Claude-based re-ranking into a SQLite-backed, resumable workflow.
...
- **CLI** (`src/vidpipe/`): Typer CLI with Rich progress bars. Each stage reads/writes a single SQLite DB (`data/index.db`). Stages are idempotent and resumable.
...
```
src/vidpipe/           # Python CLI package
...
# Bulk ingest + process
uv run vidpipe ingest /archive --workers 8
uv run vidpipe extract-audio --workers 4
uv run vidpipe transcribe
uv run vidpipe detect-events
uv run vidpipe score
uv run vidpipe rank              # optional; needs `claude` CLI on PATH

# Cut a single clip after UI review
uv run vidpipe create-clip <clip_id>
```
...
```bash
docker compose exec cli uv run vidpipe ingest /archive --workers 8
```
...
- **Paths**: Use `pathlib.Path`. All runtime artifacts resolve through `config.py` (env-overridable via `VIDPIPE_DATA`, `VIDPIPE_MODELS`).
...
|----------|---------|---------|
| `VIDPIPE_DATA` | `./data` | DB + artifacts root |
| `VIDPIPE_MODELS` | `./models` | Model cache |
| `VIDPIPE_WHISPER_MODEL` | `large-v3` | Whisper model size |
| `VIDPIPE_WHISPER_COMPUTE` | `float16` | `int8` for small GPUs |
| `VIDPIPE_CLIP_VCODEC` | `libsvtav1` | Video codec for cuts |
| `VIDPIPE_CLIP_CRF` | `28` | Quality (0=lossless, 63=worst) |
| `VIDPIPE_CLAUDE_CLI` | `claude` | Path to Claude Code CLI |
| `VIDPIPE_CLAUDE_MODEL` | `sonnet` | Model forwarded to `claude -p` |
| `VIDPIPE_BASE_URL` | inferred from request | Public origin for share links (e.g. `https://clips.example.com`) |
...
- **Clips are generated on-demand.** The pipeline identifies moments but does not cut mp4s by default. Users create clips via the UI editor or `vidpipe create-clip <id>`.
...
- **Per-video reprocess** resets stage flags for a single video and re-runs from that stage onward. Available in the UI or via `vidpipe reprocess-video <id> <stage>`.
...
- **Share links** are served server-side at `/share/:id` with Open Graph and Twitter Card meta tags for inline video playback on Discord, Twitter, etc. Set `VIDPIPE_BASE_URL` when running behind a reverse proxy so absolute URLs are correct.
